...
def read_csv_with_retries(uploaded_file):
    ...
    Tries different skiprows to find correct header, and delimiter fallback.
    """
    attempts = [
        {"skiprows": None, "sep": None},  # default
        {"skiprows": 1, "sep": None},
        {"skiprows": 2, "sep": None},
        {"skiprows": 3, "sep": None},
        # delimiter fallbacks (rare, but cheap to try)
        {"skiprows": None, "sep": ";"},   # some exports use semicolon
        {"skiprows": 1, "sep": ";"},
        {"skiprows": 2, "sep": ";"},
    ]
    ...
    for a in attempts:
        try:
            ...
            # If it's totally empty, keep trying
            if df_try is None or df_try.shape[0] == 0 and df_try.shape[1] == 0:
                continue

            # If columns are Unnamed, header row probably still wrong
...
# -----------------------------
try:
    df, used_params = read_csv_with_retries(uploaded)
except Exception as e:
    st.error("Could not read the CSV. This usually means the file isn't a valid CSV or has an unusual format.")
    st.exception(e)
    st.stop()
...
col_xslg   = find_col(df.columns, ["xSLG", "x_slg", "expected slugging", "xslg"])

# If still not found, show debug help
if col_xslg is None:
    st.error("Couldn't find xSLG column. Your CSV loaded, but the header may not match expected names.")
    st.write("CSV read parameters used:", used_params)
...
col_whf  = find_col(df.columns, ["Whiff %", "Whiff%", "SwStr%", "SwingingStrike%"])
...
with tab_plots:
    ...
    if not candidates:
        ...
        st.pyplot(scatter_fig(df_num, xcol, col_xslg, f"{col_xslg} vs {xcol}"))
        ...
        for i, qc in enumerate(quick):
            with cols[i % 2]:
                st.pyplot(scatter_fig(df_num, qc, col_xslg, f"{col_xslg} vs {qc}"))
